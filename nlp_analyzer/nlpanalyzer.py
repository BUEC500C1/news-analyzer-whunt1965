...
def analyze_sentiment(text):
    logging.info(f"{{Event: {Event.AnalyzeSentiment_Initiated}, Target: {text}}}")
    if type(text) is str:
        # Insert call to helper
        logging.info(f"{{Event: {Event.AnalyzeSentiment_Success}, Target: {text}}}")
        return 1 #hard coded result
    else:
        logging.error(f"{{Event: {Event.AnalyzeSentiment_Error}, Target: {text}}}")
        return ""
...
def analyze_entity(text):
    logging.info(f"{{Event: {Event.AnalyzeEntity_Initiated}, Target: {text}}}")
    if type(text) is str:
        # Insert call to helper
        logging.info(f"{{Event: {Event.AnalyzeEntity_Success}, Target: {text}}}")
        return ["John Cooper"] #hard coded result
    else:
        logging.error(f"{{Event: {Event.AnalyzeEntity_Error}, Target: {text}}}")
        return []
...
def analyze_entity_sentiment(text):
    logging.info(f"{{Event: {Event.AnalyzeEntitySentiment_Initiated}, Target: {text}}}")
    if type(text) is str:
        # Insert call to helper
        logging.info(f"{{Event: {Event.AnalyzeEntitySentiment_Success}, Target: {text}}}")
        return [{"Entity": "John Doe", "Score": 9}]
    else:
        logging.error(f"{{Event: {Event.AnalyzeEntitySentiment_Error}, Target: {text}}}")
        return []
...
def classify_content(text):
    logging.info(f"{{Event: {Event.ClassifyContent_Initiated}, Target: {text}}}")
    if type(text) is str:
        # Insert call to helper
        logging.info(f"{{Event: {Event.ClassifyContent_Success}, Target: {text}}}")
        return {'/EC530': .9} #Hard code for now
    else:
        logging.error(f"{{Event: {Event.ClassifyContent_Error}, Target: {text}}}")
        return {}
