...
def create(myFile):
    logging.info(f"{{action: {Event.Upload_Initiated}, Target: {myFile}}}")
    try:
        # Insert call to create helper
        logging.info(f"{{action: {Event.Upload_Success}, Target: {myFile}}}")
        return 'File Successfully Uploaded'
    except:
        logging.error(f"{{action: {Event.Upload_Error}, Target: {myFile}}}")
...
def read(myFile, *attrs):
    ...
    try:
        ...
        logging.info(f"{{action: {Event.Read_Success}, Target: {myFile, attrs}}}")
        return '<Results...>'
    except:
        logging.error(f"{{action: {Event.Read_Error}, Target: {myFile, attrs}}}")
...
def update(myFile, *attrs):
    ...
    try:
        ...
        logging.info(f"{{action: {Event.Update_Success}, Target: {myFile, attrs}}}")
        return 'Update Successful'
    except:
        logging.error(f"{{action: {Event.Update_Error}, Target: {myFile, attrs}}}")
...
def delete(myFile, *attrs):
    ...
    try:
        ...
        logging.info(f"{{action: {Event.Delete_Success}, Target: {myFile, attrs}}}")
        return 'Deletion Successful'
    except:
        logging.error(f"{{action: {Event.Delete_Error}, Target: {myFile, attrs}}}")
